...
class TestSearchBranches:
    ...
    @pytest.mark.asyncio
    @patch('openhands.app_server.git.git_router.ProviderHandler')
    async def test_search_query_supports_pagination(self, mock_handler_cls):
        """Test that branch search queries support page_id pagination."""
        mock_handler = MagicMock()
        mock_handler.search_branches = AsyncMock(
            return_value=[
                Branch(name='feature-31', commit_sha='abc123', protected=False),
                Branch(name='feature-32', commit_sha='def456', protected=False),
            ]
        )
        mock_handler_cls.return_value = mock_handler
        ...
        assert result.items[0].name == 'feature-31'
        assert result.next_page_id == encode_page_id(3)
        mock_handler.search_branches.assert_called_once_with(
