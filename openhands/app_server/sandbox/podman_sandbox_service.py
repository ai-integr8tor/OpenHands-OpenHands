...
from openhands.app_server.sandbox.podman_sandbox_spec_service import get_podman_client
...
@dataclass
class PodmanSandboxService(SandboxService):
    """Sandbox service built on podman.

    The Podman API does not currently support async operations, so some of these operations will block.
    Given that the podman API is intended for local use on a single machine, this is probably acceptable.
    """
    ...
    podman_client: podman.PodmanClient = field(default_factory=get_podman_client)
    ...
    def _podman_status_to_sandbox_status(self, podman_status: str) -> SandboxStatus:
        """Convert Podman container status to SandboxStatus."""
        status_mapping = {
            'running': SandboxStatus.RUNNING,
            'paused': SandboxStatus.PAUSED,
            # The stop button was pressed in the podman console
        ...
        return status_mapping.get(podman_status.lower(), SandboxStatus.ERROR)
    ...
    async def _container_to_sandbox_info(self, container) -> SandboxInfo | None:
        """Convert Podman container to SandboxInfo."""
        # Convert Podman status to runtime status
        status = self._podman_status_to_sandbox_status(container.status)
    ...
    async def _container_to_checked_sandbox_info(self, container) -> SandboxInfo | None:
        ...
        if (
            ...
            try:
                # When running in Podman, replace localhost hostname with host.docker.internal for internal requests
                ...
                # Get the started_at from the podman container info and fallback to sandbox created_at
                try:
    ...
    async def search_sandboxes(
        ...
        try:
            ...
            all_containers = self.podman_client.containers.list(all=True, compatible=True)
            sandboxes = []
    ...
    async def get_sandbox(self, sandbox_id: str) -> SandboxInfo | None:
        ...
        try:
            if not sandbox_id.startswith(self.container_name_prefix):
                return None
            container = self.podman_client.containers.get(sandbox_id, compatible=True)
    ...
    async def start_sandbox(
        ...
        network_mode = 'host' if self.use_host_network else 'bridge'
        ...
        devices = ['/dev/kvm:/dev/kvm:rwm'] if self.kvm_enabled else []
        ...
        try:
            ...
            container = self.podman_client.containers.run(  # type: ignore[call-overload,misc]
                ...
                labels=labels,
                detach=True,
                # Use Podman's tini init process to ensure proper signal handling and reaping of
                ...
                idmappings={
                    'UIDMap': [
                        {'container_id': 0, 'host_id': 1, 'size': 10001},
                        {'container_id': 10001, 'host_id': 0, 'size': 1},
                        {'container_id': 10002, 'host_id': 10002, 'size': 55535},
                    ],
                    'GIDMap': [
                        {'container_id': 0, 'host_id': 1, 'size': 10001},
                        {'container_id': 10001, 'host_id': 0, 'size': 1},
    ...
    async def delete_sandbox(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                volume = self.podman_client.volumes.get(volume_name)
                volume.remove()
            except (NotFound, APIError):
...
class PodmanSandboxServiceInjector(SandboxServiceInjector):
    """Dependency injector for podman sandbox services."""
